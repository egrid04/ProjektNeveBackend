...
namespace ProjektNeveBackend.Controllers
{
    ...
    public class FileUploadController : ControllerBase
    {
        ...
        public async Task<IActionResult> FileUploadFtp(string category)
        {
            try
            {
                var httpRequest = Request.Form;
                var postedFile = httpRequest.Files[0];
                string fileName = postedFile.FileName;
                string subFolder = "/"+category+"/";

                var url = "ftp://ftp.nethely.hu" + subFolder + "/" + fileName;
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create(url);
                request.Credentials = new NetworkCredential("Vizsgaremek", "DepressziosDominik");
